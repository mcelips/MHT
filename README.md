...
* Как исключение, подключение к базе данных можно оставить как `$connection`, не переименовывая `$_CONNECTION_`.
...
* Проверку сессии на пустоту `if ($ses == "") { getError(0); }` можно вынести из основного файла в функцию проверки
  ...
  функции в разделе «Проверка авторизации пользователя по сессии».
